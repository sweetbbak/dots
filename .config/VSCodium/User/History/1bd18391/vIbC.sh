...
get_images() {
    ...
    cd "$dl" && gum spin --show-output --title "Downloading ${title}" -- wget -p "$dl" -q -U "Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:49.0) Gecko/20100101 Firefox/49.0" \
...
gum spin -s line --title "updating links" get_html "$website"
# fi
# rm index.html
temp=$(mktemp)
...
choice="$(fzf --cycle --with-nth 4 --delimiter '/' < "$links")"

choice=$(echo -e "${choice}" | tr -d '[:space:]')
gum style --border thick --border-foreground 212 --underline --foreground 212 --bold "$choice"


title="$(echo "$choice" | cut -d "/" -f4)"
dl=$(mkdir -p "$asura""$title")
cd "$asura""$title" || echo "dir not detected" && exit
#get_images "$choice"
wget -U "Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:49.0) Gecko/20100101 Firefox/49.0" -nd -r --level=1  -e robots=off -A jpg,jpeg -H "$choice"

ouch compress "$(\ls -1v *.jpg)" "$title".zip && mv "$title".zip "$title".cbz