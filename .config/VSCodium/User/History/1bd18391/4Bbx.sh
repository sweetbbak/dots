...
get_images() {
    ...
    title=$(echo "$1" | cut -d "/" -f4 | sed 's/-chapter-.*//' | tr '-' ' ')
    mkdir -p "$asura"
    gum spin --show-output --title "Downloading ${title}" -- wget -q -U "Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:49.0) Gecko/20100101 Firefox/49.0" \
