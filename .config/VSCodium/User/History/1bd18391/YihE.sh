...
dl=$(mkdir -p "$asura""$title" && cd "$asura""$title")
#get_images "$choice"
(cd "$asura""$title" && gum spin wget -q -U "Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:49.0) Gecko/20100101 Firefox/49.0" -nd -r --level=1  -e robots=off -A jpg,jpeg -H "$choice" && ouch compress "$(\ls -1v *.jpg)" "$title".zip && mv "$title".zip "$title".cbz)